...
def idno_already_exists(idno):
        return check_idno_in_database(idno)

def check_idno_in_database(idno):
    db = db_connect()
    cursor = db.cursor()
    cursor.execute("SELECT idno FROM student WHERE idno =  %s", (idno,))
