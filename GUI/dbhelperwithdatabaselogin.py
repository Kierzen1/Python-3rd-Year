...
def showTableStudent(table, keys, vals, tree):
    db:object = db_connect()  # Replace with your actual database file
    cursor = db.cursor()
    sql = f"SELECT * FROM student"
    cursor.execute(sql, (vals[0], vals[1]))
    rows = cursor.fetchall()
    
    for row in rows:
        print(row)  # This will print the rows to the console
