...
def updateStudent() -> None:
    ...
    for student in studentlist:
        fields = student.strip().split(",")
        if index in updater_id_ln[0] or index in updater_id_ln[1]:
            print("Student Found:")
            ...
            updateidno = input("Enter new IDNO:")
    ...
    if updated:
        f.close()  # Close the file before reopening in write mode
        f = open(filename, "w")
        f.writelines(update_student_data)
        f.close()  # Close the file after writing
        print("Student Record updated successfully")
    else:
        f.close()  # Close the file if no update was made
        print("Student not found")
